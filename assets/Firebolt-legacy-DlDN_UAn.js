    ...
    return {
      ...
      execute: function execute() {
        ...
        function event(module, event, value) {
          var listener = Object.entries(eventMap).find(_ref => {
        ...
        function dotGrab$1() {
          ...
          var _loop = function _loop(i) {
            ref = (Object.entries(ref).find(_ref3 => {
        ...
        function mock(module, method, params, value, contextParameterCount, def) {
          ...
          var hash = contextParameterCount ? '.' + Object.keys(params).filter(key => key !== 'value').map(key => params[key]).join('.') : '';
          var key = `${module}.${method}${hash}`;
          if (type === 'getter') {
        ...
        class WebsocketTransport {
          ...
          _notifyCallbacks(message) {
            ...
            var _loop2 = function _loop2(i) {
              setTimeout(() => _this._callbacks[i](message), 1);
            };
        ...
        */
        ...
        class Transport {
          ...
          _batch(requests) {
            ...
            requests.forEach(_ref5 => {
              ...
              var result = this._processRequest(module, method, params, transforms);
              results.push({
          ...
          _addPromiseToQueue(module, method, params, transforms) {
            return new Promise((resolve, reject) => {
              ...
              var deprecated = this._deprecated[module.toLowerCase() + '.' + method.toLowerCase()];
              if (deprecated) {
        ...
        var registerEventContext = (module, event, context) => {
        ...
        var doListen = function doListen(module, event, callback, context, once) {
          ...
          if (typeof callback !== 'function') {
            ...
            var contextKey = Object.keys(context).sort().map(key => key + '=' + JSON.stringify(context[key])).join('&');
            listenerId++;
            if (once) {
              oncers.push(listenerId);
            }
            events.forEach(event => {
              var key = module + '.' + event + (hasContext ? `.${contextKey}` : '');
        ...
        var getListenArgs = function getListenArgs() {
          ...
          var _getClearArgs = getClearArgs(...args),
        ...
        var once$3 = function once$3() {
          var _getListenArgs = getListenArgs(...arguments),
        ...
        var listen$3 = function listen$3() {
          var _getListenArgs3 = getListenArgs(...arguments),
        ...
        var clear$3 = function clear$3() {
          ...
          if (args && args.length && typeof args[0] === 'number') {
            ...
            var _getClearArgs3 = getClearArgs(...args),
        ...
        var prioritize = function prioritize() {
          var _getListenArgs5 = getListenArgs(...arguments),
        ...
        var doClear = function doClear() {
          ...
          if (typeof moduleOrId === 'number') {
            ...
            if (!moduleOrId && !event) {
              ...
              var hasContext = Object.values(context).length > 0;
              var contextKey = Object.keys(context).sort().map(key => key + '=' + JSON.stringify(context[key])).join('&');
              var _key4 = moduleOrId + '.' + event + (hasContext ? `.${contextKey}` : '');
        ...
        var Events = {
          ...
          broadcast(event, value) {
            emit(Object.entries(keys).find(_ref6 => {
          ...
          _ready = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
            ...
            return _regeneratorRuntime().wrap(function _callee$(_context) {
              while (1) switch (_context.prev = _context.next) {
                case 0:
                  _context.next = 2;
                  return prioritize('Lifecycle', (event, value) => {
