const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/Buttons-B5CfEONb.js","assets/Button-Cy5lsHCC.js","assets/Components-D-plLz8w.js","assets/Entity-Dyrv9Fc2.js","assets/Login-BjHOqQUx.js"])))=>i.map(i=>d[i]);
...
class CoreNode extends EventEmitter {
  ...
  update(delta, parentClippingRect) {
    ...
    if (this.updateType ^ UpdateType.All && this.updateType & UpdateType.RenderTexture) {
      for (let i = 0, length = this.children.length; i < length; i++) {
        (_a2 = this.children[i]) == null ? void 0 : _a2.setUpdateType(UpdateType.All);
      }
    }
  ...
  setRTTUpdates(type) {
    var _a2;
    this.hasRTTupdates = true;
    (_a2 = this.parent) == null ? void 0 : _a2.setRTTUpdates(type);
  }
...
class ImageWorkerManager {
  constructor(numImageWorkers2) {
    ...
    this.workers = this.createWorkers(numImageWorkers2);
...
class CoreTextureManager {
  constructor(numImageWorkers2) {
    ...
    if (this.hasCreateImageBitmap && this.hasWorker && numImageWorkers2 > 0) {
      this.imageWorkerManager = new ImageWorkerManager(numImageWorkers2);
    }
...
class TrFontManager {
   ...
   * @returns
   */
  resolveFontFace(familyMapsByPriority, props) {
...
class Stage {
  ...
  constructor(options) {
    ...
    this.options = options;
    const { canvas, clearColor, appWidth, appHeight, boundsMargin, enableContextSpy, forceWebGL2, numImageWorkers: numImageWorkers2, textureMemory, renderEngine, fontEngines } = options;
    this.eventBus = options.eventBus;
    this.txManager = new CoreTextureManager(numImageWorkers2);
  ...
  resolveNodeDefaults(props) {
    ...
    return {
      ...
      strictBounds: (_Y = props.strictBounds) != null ? _Y : false
    };
  }
}
class RendererMain extends EventEmitter {
  ...
  constructor(settings, target) {
    ...
    const resolvedSettings = {
      ...
      forceWebGL2: (_g = settings.forceWebGL2) != null ? _g : false,
      inspector: (_h = settings.inspector) != null ? _h : false,
      renderEngine: settings.renderEngine,
      quadBufferSize: (_i = settings.quadBufferSize) != null ? _i : 4 * 1024 * 1024,
      fontEngines: settings.fontEngines
    };
    this.settings = resolvedSettings;
    const { appWidth, appHeight, deviceLogicalPixelRatio: deviceLogicalPixelRatio2, devicePhysicalPixelRatio, inspector } = resolvedSettings;
...
class WebGlCoreRenderer extends CoreRenderer {
  ...
  addQuad(params2) {
    ...
    let texture = params2.texture || this.defaultTexture;
...
function flattenStyles$1(obj, result = {}) {
  if (isArray(obj)) {
    obj.forEach((item) => {
      flattenStyles$1(item, result);
    });
  } else if (obj) {
...
function logRenderTree(node) {
  ...
  tree.forEach((node2, i) => {
    if (!node2._rendererProps) {
      return;
    }
    node2._rendererProps.parent = void 0;
    node2._rendererProps.shader = void 0;
    const props = JSON.stringify(node2._rendererProps, null, 2);
    const effects = node2._effects ? "props".concat(i, ".shader = convertEffectsToShader(").concat(JSON.stringify(node2._effects, null, 2), ");") : "";
    const parent2 = i === 0 ? "rootNode" : "node".concat(i - 1);
    output += "\nconst props".concat(i, " = ").concat(props, ";\nprops").concat(i, ".parent = ").concat(parent2, ";\n").concat(effects, "\nconst node").concat(i, " = renderer.createNode(props").concat(i, ");\n");
  });
  return output;
}
  ...
  if (justify === "flexStart") {
    ...
    if (node.flexBoundary !== "fixed") {
      ...
      if (calculatedSize !== node[dimension]) {
        node["preFlex".concat(dimension)] = node[dimension];
...
const shaderCache = /* @__PURE__ */ new Map();
function convertEffectsToShader(node, styleEffects) {
  let cacheKey;
  if (Config.enableShaderCaching) {
    const roundedAlpha = Math.round((node.alpha || 0) * 10) / 10;
    cacheKey = "".concat(node.width, ",").concat(node.height, ",").concat(roundedAlpha) + JSON.stringify(styleEffects);
  ...
  set style(values) {
    ...
    if (isArray(values)) {
      ...
      if (v.length === 0) {
        ...
        this._style = isArray(v[0]) ? flattenStyles$1(v[0]) : v[0];
      } else {
        this._style = flattenStyles$1(v);
      }
    } else {
      this._style = values;
    }
...
function createResource(pSource, pFetcher, pOptions) {
  ...
  function loadEnd(p, v, error2, key2) {
    if (pr === p) {
      ...
      if ((p === initP || v === initP) && options.onHydrated) queueMicrotask(() => options.onHydrated(key2, {
        value: v
      }));
      initP = NO_INIT;
...
function createSelector(source, fn = equalFn, options) {
  ...
  const node = createComputation((p) => {
    const v = source();
    for (const [key2, val] of subs.entries()) if (fn(key2, v) !== fn(key2, p)) {
...
function combineStyles(...styles2) {
  return untrack(() => flattenStyles(styles2));
}
...
class SdfTextRenderer extends TextRenderer {
  ...
  resolveFontFace(props) {
    return this.stage.fontManager.resolveFontFace(this.fontFamilyArray, props);
  }
  /**
...
class CanvasTextRenderer extends TextRenderer {
  constructor(stage) {
    ...
    __publicField(this, "loadFont", (state) => {
      ...
      const trFontFace = this.stage.fontManager.resolveFontFace(this.fontFamilyArray, state.props);
...
function getChildrenByPosition(node, x, y) {
  ...
  while (queue.length > 0) {
    ...
    for (const currentNode of queue) {
      if (isElementNode(currentNode) && currentNode.alpha !== 0 && !currentNode.skipFocus && testCollision(
        x,
        y,
        currentNode.lng.absX || 0 * precision,
        currentNode.lng.absY || 0 * precision,
    ...
    if (size === 1) {
      ...
      let maxZIndex = -1;
      for (const node2 of currentLevelNodes) {
        const zIndex = (_b = node2.zIndex) != null ? _b : -1;
        if (zIndex > maxZIndex) {
          maxZIndex = zIndex;
  ...
  createEffect(() => {
    if (scheduled()) {
      ...
      if (result.length) {
        ...
        const activeElmParent = activeElm == null ? void 0 : activeElm.parent;
        if (activeElm && (activeElmParent == null ? void 0 : activeElmParent.selected) !== void 0) {
          activeElmParent.selected = activeElmParent.children.indexOf(activeElm);
        }
        activeElm == null ? void 0 : activeElm.setFocus();
      }
    }
  });
}
function LazyUp(props) {
  ...
  const isRow = createMemo(() => {
    var _a2;
    return others.direction === "row" || ((_a2 = others.style) == null ? void 0 : _a2.flexDirection) === "row" || others.flexDirection === "row";
  });
...
function withScrolling(isRow) {
  ...
  return (selected, componentRef, selectedElement, lastSelected) => {
    ...
    if (!componentRef.children.length)
      return;
    const gap = componentRef.gap || 0;
    const scroll2 = componentRef.scroll || "auto";
    const rootPosition = (_b = (_a2 = componentRef._targetPosition) != null ? _a2 : componentRef[axis]) != null ? _b : 0;
    ...
    const movement = lastSelected === void 0 ? "incremental" : lastSelected - 1 === selected ? "decremental" : "incremental";
    let nextPosition = rootPosition;
    const [lastItem, containerSize] = updateLastIndex(isRow, componentRef);
    const isNotShown = (pos, size) => Math.abs(rootPosition - offset) + containerSize < pos + size;
    if (scroll2 === "auto") {
    ...
    } else if (scroll2 === "always" || scroll2 === "edge" && movement === "decremental" && Math.abs(rootPosition) > selectedPosition) {
      nextPosition = -selectedPosition + offset;
    } else if (scroll2 === "edge" && movement === "incremental" && isNotShown(selectedPosition, selectedSize)) {
      nextPosition = rootPosition - selectedSize - gap;
    }
...
function updateLastIndex(isRow, items) {
  ...
  if (isRow) {
    return [
      {
        position: (_a2 = lastChild.x) != null ? _a2 : 0,
        size: (_b = lastChild.width) != null ? _b : 0
      },
      (_c = items.preFlexwidth || items.width) != null ? _c : 0
    ];
  }
  return [
    {
    ...
    (_f = items.preFlexheight || items.height) != null ? _f : 0
  ];
}
...
const Container$6 = makeComponentStyles(container$d);
...
const Column = (props) => {
  const mergedStyle = createMemo(() => {
    var _a2;
    return [props.style, styles$6.Container.tones[(_a2 = props.tone) != null ? _a2 : styles$6.tone], styles$6.Container.base];
  });
...
const Row = (props) => {
  const mergedStyle = createMemo(() => {
    var _a2;
    return [props.style, styles$3.Container.tones[(_a2 = props.tone) != null ? _a2 : styles$3.tone], styles$3.Container.base];
  });
  return createComponent(View, mergeProps(props, {
    get selected() {
      return props.selected || 0;
  ...
  return createComponent(View, {
    ...
    get children() {
      return [createComponent(Text$5, {
        ...
        get alpha() {
          return props.row.type === "Hero" ? 0 : 1;
        },
        get children() {
          return props.title;
        }
        ...
        direction: "row",
        get upCount() {
          return props.row.type === "Hero" ? 3 : 10;
        },
        get each() {
          return props.items;
        },
...
const TMDB = (props) => {
  ...
  const [heroContent, setHeroContent] = createSignal({});
  useNavigate();
  let contentBlock, firstRun = true;
  const delayedBackgrounds = debounce((img) => setGlobalBackground(img), 800);
  const delayedHero = debounce((content2) => setHeroContent(content2 || {}), 600);
  createEffect(on(activeElement, (elm) => {
    if (!elm) return;
  ...
  return [createComponent(ContentBlock, {
    ref(r$) {
      var _ref$ = contentBlock;
      typeof _ref$ === "function" ? _ref$(r$) : contentBlock = r$;
    },
    y: 300,
    x: 162,
    get content() {
    ...
    get children() {
      return createComponent(LazyUp, {
        ...
        children: (row) => createComponent(TitleRow, {
          get row() {
            return row();
          },
          get title() {
...
function browsePreload({ params: params2 }) {
  return createMemo(() => {
    return browseProvider(params2.filter || "all");
  });
}
...
const Grid = lazy(() => __vitePreload(() => import("./Grid-Bxyl_UUi.js"), true ? [] : void 0));
const Portal = lazy(() => __vitePreload(() => import("./Portal-CtzaQbOw.js"), true ? [] : void 0));
const TextPage = lazy(() => __vitePreload(() => import("./Text-C_ANi9vw.js"), true ? [] : void 0));
const CreatePage = lazy(() => __vitePreload(() => import("./Create-BrSvY-_n.js"), true ? [] : void 0));
const ViewportPage = lazy(() => __vitePreload(() => import("./Viewport-BOCOb3EO.js"), true ? [] : void 0));
const ButtonsPage = lazy(() => __vitePreload(() => import("./Buttons-B5CfEONb.js"), true ? __vite__mapDeps([0,1]) : void 0));
const PositioningPage = lazy(() => __vitePreload(() => import("./Positioning-Cbrrlq8j.js"), true ? [] : void 0));
const TransitionsPage = lazy(() => __vitePreload(() => import("./Transitions-B8gty81v.js"), true ? [] : void 0));
const ComponentsPage = lazy(() => __vitePreload(() => import("./Components-D-plLz8w.js"), true ? __vite__mapDeps([2,1]) : void 0));
const FocusHandlingPage = lazy(() => __vitePreload(() => import("./FocusHandling-EvbLcxJ-.js"), true ? [] : void 0));
const GradientsPage = lazy(() => __vitePreload(() => import("./Gradients-CODeVGSd.js"), true ? [] : void 0));
const FlexPage = lazy(() => __vitePreload(() => import("./Flex-BShYuwSv.js"), true ? [] : void 0));
const FlexSizePage = lazy(() => __vitePreload(() => import("./FlexSize-C5a4rEWn.js"), true ? [] : void 0));
const FlexColumnSizePage = lazy(() => __vitePreload(() => import("./FlexColumnSize-7s5cc7KP.js"), true ? [] : void 0));
const FlexColumnPage = lazy(() => __vitePreload(() => import("./FlexColumn-BSe4X3Hm.js"), true ? [] : void 0));
const ButtonsMaterialPage = lazy(() => __vitePreload(() => import("./ButtonsMaterial-MPfV2yde.js"), true ? [] : void 0));
const SuperFlexPage = lazy(() => __vitePreload(() => import("./SuperFlex-BV3hegRX.js"), true ? [] : void 0));
const Entity = lazy(() => __vitePreload(() => import("./Entity-Dyrv9Fc2.js"), true ? __vite__mapDeps([3,1]) : void 0));
const People = lazy(() => __vitePreload(() => import("./People-D5tvOkDK.js"), true ? [] : void 0));
const LoginPage = lazy(() => __vitePreload(() => import("./Login-BjHOqQUx.js"), true ? __vite__mapDeps([4,1]) : void 0));
