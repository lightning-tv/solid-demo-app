...
export function onKeyPressWhenKeyboardOpen(setKeyEvent: Setter<string>, event: KeyboardEvent) {
  if (event.key.length === 1) setKeyEvent(event.key);
  else if (event.key === "Backspace") setKeyEvent("delete");
  if ((event.key.length === 1 && /[a-zA-Z0-9._@-]/.test(event.key)) || event.key === "Backspace") return true;
  return false;
}
...
export const ActionKey: Component<NodeProps & { key: any }> = (props) => (
  <Switch>
    <Match when={typeof props.key === "string"}>
      <View {...props} key={props.key} display="flex" padding={20} style={keyContainerStyle}>
...
export const Keyboard: Component<NodeProps & { formats?: any; onEnter: any }> = (props) => {
  ...
  return (
    <Column transition={false} {...props} gap={12} plinko scroll="none" onEnter={handleEnter}>
      <Index each={config()}>
        {(keyRow) => (
          <Row gap={6} justifyContent="center" scroll="none">
            <Index each={keyRow()}>
              {(key) => (
                <Show
                  when={typeof key() === "string" && (key() as string).length === 1}
...
export const FullScreenKeyboard: Component<
  NodeProps & { type: any; valueSignal: Signal<string>; placeholder?: string }
> = (props) => {
  const keyEvents = createSignal("");
  const [_keyEvent, setKeyEvent] = keyEvents;

  const onEnter = (_e, _keyboard, key) => {
    if (key.key === "save" || key.key === "Save") return false;

    if (key.key) setKeyEvent(key.key as string);
