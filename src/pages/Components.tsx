...
function Square(props) {
  return (
    <View {...props} width={props.size || 80} height={props.size || 80} color={props.color || "#e0e0e0"} />
  );
}
...
const PositioningPage = () => {
  ...
  return (
    <View x={150} autofocus>
      ...
      <Visible when={size()}>
        <Square onDestroy={() => console.log("destroyed")} x={100} y={100} size={50} color="#ef4444" />
      ...
      <View x={1000} y={100}>
        ...
        <For each={dynamicComponents()}>
          {(Component, index) => <Dynamic component={Component} x={205 * index()} y={300} size={50} />}
        </For>
      </View>
    </View>
  );
